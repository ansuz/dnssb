...
var answer = function (sbot, req, res, opt) {
    ...
    Query.query(sbot, q, false, function (err, records) {
        if (err) {
        ...
        if (opt && opt.verbose) {
            ...
            console.log("%s (%s) => %s", q.name, types, vals);
        }
        // TODO: set NXDOMAIN if there are no records of any type for the
        // requested name (including via a wildcard), and there is a SOA record
        // for one of the parent domains of the requested domain.
        res.answer = records;
        res.end();
    });
};
