...
var validateRecord = Publish.validateRecord = function (record) {
    ...
    // TODO rename 'value' to 'data' ???
    // TODO perform stricter validation on value
    if (!(record.value && typeof(record.value) === 'string')) { return "[Record DataError] expected a value to publish"; }
};
...
var makeRecord = Publish.makeRecord = function (name, type, value, _class) {
    return {
        ...
        type: type,
        value: value, // TODO change to 'data' ???
        class: _class || 'IN',
    };
};
