...
Query.querySingle = function (sbot, question, cb) {
    ...
    Pull.filter(function (record) {
        ...
        if (isIncrementalTransfer
          && zoneSerials.getSerial(question.name) < question.serial) {
            return false
        }
        var nameMatches = isTransfer ? question.name in recordDomains :
               record.type === 'SOA' ? record.name in authorityDomains :
