...
Query.querySingle = function (sbot, question, cb) {
    ...
    Query.drainSet(function (record) {
        ...
        result.authorities.forEach(updateSOA);
        result.answers.forEach(updateSOA);
        function updateSOA(r) {
        ...
        if (isTransfer) {
            ...
            if (soa) {
                result.answers = [soa].concat(
                    result.answers.filter(function (r) { return r !== soa; }),
                    [soa]
                );
            }
        } else {
            ...
            result.answers = result.answers.map(function (r) {
                return r.name !== qName
                    && r.name in wildcardDomains ? {
                        name: qName,
                        type: r.type,
                        class: r.class,
                        data: r.data,
