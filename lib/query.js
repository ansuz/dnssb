...
function removeTrailingDot(domain) {
...
Query.querySingle = function (sbot, question, cb) {
    ...
    Pull(Query.all(sbot),
    Pull.filter(function (record) {
        var rName = removeTrailingDot(record.name)
        var recordDomains = expandName(rName);
        ...
        var nameMatches = isTransfer
            ? qName in recordDomains
            : rName in wildcardDomains;
        if (nameMatches) {
    ...
    Query.drainSet(function (record) {
        if (record.type === 'NS' || record.type === 'SOA') {
            ...
            if (question.class === record.class
              && question.type === record.type
              && removeTrailingDot(record.name) in wildcardDomains) {
